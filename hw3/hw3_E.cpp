...
int main() {
    ...
    const auto seg_size = sorted.size();

    auto get_idx = [&](int val) {
        return static_cast<size_t>(upper_bound(sorted.begin(), sorted.end(), val) - sorted.begin());
    };
    SegmentTree<int> seg_tree(seg_size);
    auto ans = 0LL;

    for (const auto &i : arr) {
        auto idx = get_idx(i);
        ans += (idx == seg_size ? 0 : seg_tree.count(idx, seg_size));
        seg_tree.increment(idx - 1, 1);
    }
